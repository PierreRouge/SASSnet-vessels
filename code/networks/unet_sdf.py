...
class TinyDecoder_UNet(nn.Module):
    ...
    def __init__(self, dim, in_channel, features, strides, kernel_size, nclasses):
        ...
        self.final_conv_2 = nn.Conv3d(features[0], nclasses, kernel_size=1)
        self.tanh = nn.Tanh()
    ...
    def forward(self, x4: torch.Tensor, x1: torch.Tensor, x2: torch.Tensor, x3: torch.Tensor):
        ...
        out_tanh = self.tanh(x_final)
        
        out_seg = self.final_conv_2(x7)
...
class TinyUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        out_tanh, out_seg = self.decoder(x4, x1, x2, x3)
        return out_tanh, out_seg
